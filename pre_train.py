...
from model.chat_model import SmallT5Model,SmallLlamaModel
from model.dataset import MyDataset
from config import TrainConfig, T5ModelConfig

from utils.functions import json_to_dataclass, get_T5_config,get_Llama_config, MyTrainerCallback
...
#         input_ids = [np.array(item, dtype=np.uint16) for item in encoded_response["input_ids"]]
#         labels = [np.array(item[1:] + [eos_token_id], dtype=np.uint16) for item in encoded_response["input_ids"]]
