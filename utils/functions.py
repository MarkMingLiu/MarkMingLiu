...
def get_Llama_config(config: T5ModelConfig, vocab_size: int, decoder_start_token_id: int=0, eos_token_id: int=1) -> LlamaConfig:
    '''
    用户配置转换为
    '''
    llama_config = LlamaConfig()
    # llama_config.model_type = 'TextToTextModel'
    # 初始化
    llama_config.intermediate_size = config.d_ff
    # llama_config.num_key_value_heads = config.d_kv
    llama_config.num_key_value_heads = config.num_heads
    llama_config.hidden_size = config.d_model
    # llama_config.num_hidden_layers = config.num_decoder_layers
    llama_config.num_attention_heads = config.num_heads
    llama_config.num_hidden_layers = config.num_layers
    llama_config.vocab_size = vocab_size
    llama_config.decoder_start_token_id = decoder_start_token_id
    llama_config.eos_token_id = eos_token_id
